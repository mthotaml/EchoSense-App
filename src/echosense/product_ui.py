...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    .factor-info { position:relative; display:inline-grid; place-items:center; width:18px; height:18px; padding:0; border:1px solid var(--line); border-radius:50%; background:transparent; color:var(--muted); font:700 11px/1 system-ui,sans-serif; text-transform:none; cursor:help; }
...
<body>
  ...
  <main>
    ...
    <section id="listening-controls" class="panel control-center"><div class="control-center-header"><div><div class="eyebrow blue">Listening controls</div><h2>Personalize what EchoSense plays next</h2><p class="copy">Choose your current moment, tune recommendation priorities, and manage live context from one place.</p></div></div><div class="control-groups"><section id="moment-panel" class="control-group"><div class="eyebrow">Listening moment</div><h3>What are you doing right now?</h3><p class="copy">EchoSense will tailor the Playback Plan to this activity and your taste.</p><label class="sr-only" for="moment">Listening moment</label><select id="moment" class="secondary moment-select" aria-label="Listening moment"><option value="general">Any moment</option><option value="driving">Driving</option><option value="working">Working</option><option value="exercising">Exercising</option><option value="relaxing">Relaxing</option><option value="social">Social</option></select><p id="moment-impact" class="moment-impact" aria-live="polite">Any moment is active. Choose an activity to verify how it changes the ranking.</p></section><section id="boost-panel" class="control-group"><div class="eyebrow blue">Recommendation priorities</div><h3>What should matter more?</h3><p class="copy">Boost Music DNA affinity, live context, learned preference, or diversity. Effective weights always remain bounded and total 100%.</p><div id="boost-controls" class="boost-grid"></div></section><section id="live-context-panel" class="control-group context-group"><div><div class="eyebrow">Live context</div><h3>Add situational signals</h3><p id="context-status" class="connection-copy">Time is automatic. Add weather, coarse location, road setting, and movement when useful.</p><div id="context-chips" class="context-chips"></div><details class="privacy-note"><summary>Privacy</summary>Location is used only to resolve current conditions; raw coordinates are not stored.</details></div><button id="context-toggle" class="secondary" type="button">Enable context</button></section></div><p id="context-statement" class="context-statement" aria-live="polite">Building the context statement used for your next track…</p></section>
  ...
  <script>
    ...
    const factorExplanations={
      ...
      'Diversity guard':'A repetition safeguard, not a recommendation-match score. It avoids repeating the same tracks or artists. Passed means this pick is not a recent duplicate and its artist is within the exposure limit.',
    ...
    async function loadLiveSpotify(moment=$('#moment').value, exclusions=[], updateCurrentPick=true, recordRound=updateCurrentPick) {
      ...
      renderBoostControls(data.effective_weights||{});renderMomentImpact(data.moment_impact);setText('#context-statement',data.context_statement||'Music DNA and the current listening moment are shaping the next track.');setText('#dna-plan-statement',`${data.moment_impact?.message||''} ${data.context_statement||'Music DNA and the current listening moment shaped this final playback order.'}`.trim());
