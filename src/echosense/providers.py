...
class FixtureMusicProvider:
    _catalog = {
        "rainy_commute": [
            RecommendationCandidate("apple_music", "fixture-rain-001", "calmer music for a rainy drive", 0.8),
            RecommendationCandidate("apple_music", "fixture-rain-002", "steady acoustic music for wet roads", 0.7),
            RecommendationCandidate("apple_music", "fixture-rain-003", "low-distraction ambient music", 0.6),
        ],
        "commute": [
            RecommendationCandidate("apple_music", "fixture-drive-001", "steady music for your drive", 0.8),
            RecommendationCandidate("apple_music", "fixture-drive-002", "focused music for commuting", 0.7),
            RecommendationCandidate("apple_music", "fixture-drive-003", "balanced road music", 0.6),
        ],
        "evening_wind_down": [
            RecommendationCandidate("apple_music", "fixture-evening-001", "lower-energy music for the evening", 0.8),
            RecommendationCandidate("apple_music", "fixture-evening-002", "gentle music for winding down", 0.7),
            RecommendationCandidate("apple_music", "fixture-evening-003", "quiet late-day listening", 0.6),
        ],
        "general_listening": [
            RecommendationCandidate("apple_music", "fixture-general-001", "music based on the available context", 0.8),
            RecommendationCandidate("apple_music", "fixture-general-002", "a broadly suitable listening option", 0.7),
            RecommendationCandidate("apple_music", "fixture-general-003", "a balanced general selection", 0.6),
        ],
    }
    ...
    def sync_library(self, user_id: str, limit: int = 25) -> list[ProviderSignal]:
        return [
            ProviderSignal("apple_music", "library_song", "fixture-library-001", "Midnight Drive", "Echo Avenue", "City Lights", "us", "fixture.library")
        ][:limit]

    def sync_recent_plays(self, user_id: str, limit: int = 25) -> list[ProviderSignal]:
        return [
            ProviderSignal("apple_music", "recent_play", "fixture-recent-001", "Rain on Glass", "Northbound", "Weather Systems", "us", "fixture.recent")
        ][:limit]
...
class AppleMusicProvider:
    ...
    def _headers(self, user_id: str | None = None, *, require_user_token: bool = False) -> dict[str, str]:
        headers = {"Authorization": f"Bearer {self.developer_tokens.token()}"}
        music_user_token = self.user_tokens.retrieve(user_id) if user_id and self.user_tokens else None
        if music_user_token:
    ...
    def _get(self, path: str, *, user_id: str | None = None, params: dict[str, object] | None = None, require_user_token: bool = False) -> dict[str, object]:
        response = httpx.get(
    ...
    def _signals(self, payload: dict[str, object], signal_type: str, source_path: str) -> list[ProviderSignal]:
